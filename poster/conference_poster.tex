...
{\bf\textsc{Super Learner for Optimal Prediction}\vspace{0.5em}} % Poster title
...
To assess the performance of Super Learner, we apply the mean squared error
(MSE), discounting observations with missing data in each of the 5 genomic
covariates of interest.

\vspace{0.5em}

That is, for each covariate $j$, observations with missing values in $y^{j}$,
and corresponding fitted values in $\hat{y}^{j}$, are removed. The MSE is then
computed as follows:
$$ MSE_{j} = \frac{1}{n_{j}} \sum_{i = 1}^{n_{j}} (\hat{y}^{j}_{i} -
 y^{j}_{i})^{2} $$
